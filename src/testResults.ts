...
class TestResults {
  ...
  private skips: any;
  // tslint:disable-next-line: no-any
  private testTotalTime: any;
  // tslint:disable-next-line: no-any
  private testExecutionTime: any;
  // tslint:disable-next-line: no-any
  private userId: any;
  // tslint:disable-next-line: no-any
  ...
  constructor(
    ...
    private tests: any,
    // tslint:disable-next-line: no-any
    runResultSummaries: any,
    // tslint:disable-next-line: no-any
    private config: any,
    private passStr = 'Pass',
    skipStr = 'Skip'
  ) {
    ...
    this.failures = this.tests.filter(
      test => test.Outcome !== passStr && test.Outcome !== skipStr
    );
    ...
    const runResultSummary =
      runResultSummaries && runResultSummaries.length > 0
        ? runResultSummaries[0]
        : null;
    ...
    this.testStartTime =
      runResultSummary !== null
        ? moment.parseZone(runResultSummary.StartTime).local()
        : this.startTime;
    // total time per run summary
    this.testTotalTime =
      runResultSummary !== null ? runResultSummary.TestTime : 0;
    ...
    this.testExecutionTime =
      runResultSummary !== null && runResultSummary.TestExecutionTime
        ...
        : tests.reduce(
            (result, test) => result + (test.RunTime ? test.RunTime : 0),
            0
          );
  ...
  get passRate() {
    return `${Math.round(
      (this.totalPassed / (this.total - this.totalSkipped)) * 100
    )}%`;
  }

  get failRate() {
    return `${Math.round(
      (this.totalFailed / (this.total - this.totalSkipped)) * 100
    )}%`;
  }
